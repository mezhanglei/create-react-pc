 ...
 *  undraggable： 不允许拖拽
 */
// export default function buildDraggableArea({ triggerAddFunc, listenAddFunc }: any = {}) {
...
// 拖拽元素的位置属性
export interface DragRectInterface {
    left: number;
    top: number;
    width: number;
    height: number;
...
export interface AddTagsProps {
    ...
    x: number;
    y: number;
    fromAreaId: string | number;
    e: MouseEvent | TouchEvent;
}

// 
export interface BuildDraggableAreaInterface {
    triggerAddFunc?: (dragRect: DragRectInterface, tag: TagInterface, e: MouseEvent | TouchEvent) => any;
    listenAddFunc?: (area: React.ReactNode, addTag: (props: AddTagsProps) => void) => void
}
...
export interface DraggableAreaRef {

}

// 返回拖拽区域的高阶函数
export default function buildDraggableArea({ triggerAddFunc = () => { }, listenAddFunc = () => { } }: BuildDraggableAreaInterface = {}) {

    // 拖拽区域
    const DraggableArea = React.forwardRef<DraggableAreaRef, DraggableAreaProps>((props, ref) => {
